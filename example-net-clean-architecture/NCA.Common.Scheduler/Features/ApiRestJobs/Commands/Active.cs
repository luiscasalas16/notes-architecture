    ...
    public class Active
    {
        ...
        public class CommandHandler : CommandHandlerBase<Command, Result>
        {
            ...
            public override async Task<Result> Handle(Command request)
            {
                ...
                if (jobKey != null)
                {
                    if (data.Status == ApiRestJob.StatusActive)
                        return Result.Failure(ApiRestErrors.JobAlreadyStatus(ApiRestJob.StatusActive));
                    else
                        return Result.Failure(ApiRestErrors.InvalidState());
                }

                await ApiRestScheduler.AddApiRestJob(scheduler, data);

                data.Status = ApiRestJob.StatusActive;
